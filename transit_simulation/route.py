...
def initialize_random_routes(sample_nr=20, simulation_date=pd.to_datetime('today'), data_path="./tests/test_data"): # if the sample_nr is less or equal to 0, all samples are used
...
def initdata_to_files(routes, schedule, data_path="./tests/test_data"):
    data_path = Path(data_path)
...
def preprocess_initdata_to_files(sample_nr=20, simulation_date=pd.to_datetime('today'), data_path='./tests/test_data'):
    data_path = Path(data_path)
...
if __name__ == "__main__":
    """Console script for HSL GTFS data preprocessing"""

    import sys
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', help="directory for data files output", type=str)
    parser.add_argument('--sample_nr', help="number of routes to pick from dataset (random sample) ", type=int, default=0)
    args = parser.parse_args()


    if args.data_dir:
        logger.info('Preprocessing GTFS to schdule.csv and routes files')
        logger.info(f'using output directory {args.data_dir}')
        preprocess_initdata_to_files(sample_nr = args.sample_nr, data_path = args.data_dir)

    # previous manual testing/experimenting script
    # theses will be executed if no arguments are given
    else:
